...
const Banner = () => {
  ...
  const [movie, setMovie] = useState([]) // 영화 정보
  const [isClicked, setIsClicked] = useState(false) // video가 있는지 없는지 여부
  ...
  const fetchData = async () => {
    ...
    // 여러 영화 중 하나의 id 가져오기
    const moveId = response.data.results[
        ...
        <div className='banner__contents'>
          ...
          <div className='banner__buttons'>
            {movie?.videos?.results[0]?.key && 
              <button
                className='banner__buttons play'
